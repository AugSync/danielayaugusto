...
const WeddingSchedule = ({
  ...
  guest: TWeddingGuest;
  isOpenedSchedule: boolean;
}) => {
  return (
    <div
      id="schedule"
      className={`relative flex w-full flex-col items-center bg-cardboard text-center ${
        isOpenedSchedule ? null : 'hidden'
      }`}
    >
      {isOpenedSchedule ? (
        <>
          <p className={'mt-6 w-full font-parisienne text-3xl text-augdi lg:text-5xl'}>
            Cronograma
          </p>
              ...
              .map(({ title, subtitle, description, idEvent }, index) =>
                guest?.[idEvent] ? (
                  <article key={index} className={`mb-6 w-full text-left `}>
                    <p className={'w-full font-parisienne text-2xl text-augdi lg:text-4xl'}>
                      {title}
                    </p>
                    <p className={'w-full font-cormorant text-lg font-normal lg:text-xl'}>
                      {subtitle}
                    </p>
